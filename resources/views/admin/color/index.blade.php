                ...
                <table class="table table-striped table-bordered bootstrap-datatable datatable">
                    ...
                    @foreach($colors as $keys => $color)
                        <tr>
                            ...
                            <td class="center">
                                @if($color->status == 1)
                                    <a href="{{route('colors.update.status',[$color->id])}}" class="btn btn-danger">
                                        <i class="halflings-icon white thumbs-down" title="Deactive status?"></i>
                                    </a>
                                @else
                                    <a href="{{route('colors.update.status',[$color->id])}}" class="btn btn-success">
                                        <i class="halflings-icon thumbs-up" title="Active status?"></i>
                                    </a>
                                @endif
                                <a href="{{route('colors.edit',$color->id)}}" class="btn btn-info">
